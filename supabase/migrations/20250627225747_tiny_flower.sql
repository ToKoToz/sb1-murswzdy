/*
  # Complete Database Reset via Supabase CLI
  ...
  5. Fixing storage policies without permission issues.
*/
...
-- Storage policies cleanup and recreation
-- Note: This section will be handled properly by Supabase CLI with admin privileges

-- Clean existing storage policies programmatically
DO $$
DECLARE
    r record;
BEGIN
    -- Only try to drop policies if we have the right to do so
    FOR r IN SELECT policyname FROM pg_policies WHERE tablename = 'objects' AND schemaname = 'storage' LOOP
        BEGIN
            EXECUTE 'DROP POLICY IF EXISTS ' || quote_ident(r.policyname) || ' ON storage.objects';
        EXCEPTION WHEN OTHERS THEN
            -- Ignore errors for policies we can't drop
            NULL;
        END;
    END LOOP;
END $$;
