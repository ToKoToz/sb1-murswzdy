...
DROP POLICY IF EXISTS "authenticated_upload_profile_pictures" ON storage.objects;
DROP POLICY IF EXISTS "authenticated_update_profile_pictures" ON storage.objects;
DROP POLICY IF EXISTS "authenticated_delete_profile_pictures" ON storage.objects;
DROP POLICY IF EXISTS "public_read_client_logos" ON storage.objects;
DROP POLICY IF EXISTS "authenticated_upload_client_logos" ON storage.objects;
DROP POLICY IF EXISTS "authenticated_update_client_logos" ON storage.objects;
DROP POLICY IF EXISTS "authenticated_delete_client_logos" ON storage.objects;
...
DROP POLICY IF EXISTS "client_logos_policy" ON storage.objects;
...
-- Handle existing users and create profiles
DO $$
DECLARE
    admin_user_id uuid;
    trainer_user_id uuid;
    admin_exists boolean := false;
    trainer_exists boolean := false;
BEGIN
    ...
    ELSE
        ...
        trainer_user_id := '44444444-4444-4444-4444-444444444444';
    ...
    -- Always create/update admin profile
    ...
    -- Always create/update trainer profile
    INSERT INTO profiles (
        id,
        name,
        email,
...
-- Create a demo training (always recreate)
INSERT INTO trainings (
    ...
    '09:00',
    '17:00',
    (SELECT id FROM profiles WHERE email = 'formateur@formation-pro.com'),
    'Marie Dubois',
    1,
    'active',
...
-- Create demo participants for the training (always recreate)
